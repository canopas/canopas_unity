...
class LeaveService {
  ...
  Future<bool> checkLeaveAlreadyApplied({
    ...
    return leaves.docs.map((doc) => doc.data()).where((leave) {
      ...
      return leaveDuration.entries.any(
        (existLeaveDay) => dateDuration.entries.any(
          (applyLeaveDay) =>
  ...
  String generateLeaveId(String spaceId) => _leaveDb(spaceId: spaceId).doc().id;
