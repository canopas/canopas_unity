...
class EmployeeLeaveBloc {
  ...
  final BehaviorSubject<List<LeaveRequestData>> _leaves =
      BehaviorSubject<List<LeaveRequestData>>();
  final BehaviorSubject<List<Employee>> _employee =
      BehaviorSubject<List<Employee>>();

  BehaviorSubject<List<LeaveRequestData>> get leaves => _leaves;
  ...
  getLeaves() async {
    List<LeaveRequestData> leaves = await _userLeaveService.getAllRequests();
    _leaves.add(leaves);
  }
  ...
  allLeaves() {
    ...
    try {
      ...
      Rx.combineLatest2(
        leaves.stream,
        employee.stream,
        (List<LeaveRequestData> leavesList, List<Employee> employees) =>
