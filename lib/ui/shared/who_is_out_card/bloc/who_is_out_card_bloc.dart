...
class WhoIsOutCardBloc extends Bloc<WhoIsOutEvent, WhoIsOutCardState> {
  ...
  WhoIsOutCardBloc(this._employeeRepo, this._leaveRepo)
    : super(
        WhoIsOutCardState(
  ...
  FutureOr<void> _fetchWhoIsOutCardLeaves(
    FetchWhoIsOutCardLeaves event,
    Emitter<WhoIsOutCardState> emit,
  ) async {
    emit(
      state.copyWith(
        status: state.selectedDayAbsences == null
            ? Status.loading
            : state.status,
    ...
    try {
      ...
      _subscription =
          getLeaveApplicationStream(
            leaveStream: _leaveRepo.leaveByMonth(
              event.focusDay ?? state.focusDay,
            ),
  ...
  List<LeaveApplication> getSelectedDateAbsences({
    ...
    required List<LeaveApplication> allAbsences,
  }) {
    return allAbsences
        .where(
          (la) =>
