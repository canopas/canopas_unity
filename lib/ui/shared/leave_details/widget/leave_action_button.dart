...
class LeaveDetailActionButton extends StatelessWidget {
  const LeaveDetailActionButton({Key? key, required this.leaveApplication}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<LeaveDetailsBloc,LeaveDetailsState>(
      ...
      builder: (context, state) => state.leaveDetailsStatus == LeaveDetailsStatus.loading?const AppCircularProgressIndicator(size: 28,):ElevatedButton(
          onPressed: (){
            context.read<LeaveDetailsBloc>().add(LeaveDetailsRemoveLeaveRequestEvent(leaveApplication));
          },
          style: ElevatedButton.styleFrom(
            backgroundColor: (leaveApplication.leave.leaveStatus == pendingLeaveStatus)
                ? AppColors.greyColor
                : AppColors.redColor,
