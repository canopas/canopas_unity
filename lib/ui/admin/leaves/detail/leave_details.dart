...
class LeaveDetailsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<LeaveDetailsBloc>()
        ..add(
...
class _LeaveDetailsViewState extends State<LeaveDetailsView> {
  @override
  Widget build(BuildContext context) {
    final userAreNotSame = context.read<LeaveDetailsBloc>().currentUserId !=
        widget.leaveApplication.leave.uid;
    final currentUserIsNotAdmin =
        !context.read<LeaveDetailsBloc>().currentUserIsAdmin;

    final showButton =
        (widget.leaveApplication.leave.leaveStatus != approveLeaveStatus ||
                widget.leaveApplication.leave.startDate >
                    DateTime.now().timeStampToInt) &&
            (context.read<LeaveDetailsBloc>().currentUserId ==
                    widget.leaveApplication.leave.uid ||
                context.read<LeaveDetailsBloc>().currentUserIsAdmin);
    ...
    return Scaffold(
      ...
      body: BlocListener<LeaveDetailsBloc, LeaveDetailsState>(
        listener: (context, state) {
          if (state.leaveDetailsStatus == LeaveDetailsStatus.success) {
            showSnackBar(
                context: context,
                msg: (widget.leaveApplication.leave.leaveStatus ==
                        pendingLeaveStatus)
      ...
      floatingActionButton: showButton
          ? LeaveDetailActionButton(leaveApplication: widget.leaveApplication)
          : null,
    );
  }
}
