...
class AdminLeavesBloc extends Bloc<AdminLeavesEvents, AdminLeavesState> {
  ...
  Future<void> _init(
    InitialAdminLeavesEvent event,
    Emitter<AdminLeavesState> emit,
  ) async {
    try {
      _members = _employeeRepo.allEmployees.toList();
      emit(
        state.copyWith(members: _members, membersFetchStatus: Status.success),
      );
      ...
      try {
        ...
        emit(
          state.copyWith(
            ...
            leaveApplicationMap: leaveApplications.groupByMonth(
              (la) => la.leave.startDate,
            ),
          ),
        );
      } on Exception {
  ...
  Future<void> _updateLeave(
    ...
    final employee = _members.firstWhereOrNull(
      (element) => element.uid == leave?.uid,
    );
    ...
    if (employee != null && leave != null) {
      ...
      leaveApplications.removeWhereAndAdd(
        leaveApplication,
