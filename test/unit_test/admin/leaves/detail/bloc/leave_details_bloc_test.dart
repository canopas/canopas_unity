...
import 'package:projectunity/ui/admin/leaves/detail/bloc/employee_leave_details_bloc.dart';
import 'package:projectunity/ui/admin/leaves/detail/bloc/leave_details_event.dart';
import 'package:projectunity/ui/admin/leaves/detail/bloc/leave_details_state.dart';
...
void main() {
  ...
  group("Leave Details Screen Test", () {
    setUp(() {
      ...
      leaveDetailsBloc =
          LeaveDetailsBloc(userLeaveService, paidLeaveService, userManager);
      loadingState = const LeaveDetailsState(
    ...
    test("fetch leave count test", () {
      ...
      leaveDetailsBloc.add(
          LeaveDetailsInitialLoadEvents(leaveApplication: leaveApplication));
      expect(
          leaveDetailsBloc.stream,
          emitsInOrder([
            loadingState,
            const LeaveDetailsState(
                leaveDetailsLeaveCountStatus:
                    LeaveDetailsLeaveCountStatus.success,
                paidLeaveCount: 12,
                remainingLeaveCount: 6.0),
          ]));
    });
    ...
    test("remove leave application test", () {
      when(userLeaveService.deleteLeaveRequest(leave.leaveId))
      ...
      expect(
          ...
          emitsInOrder([
            const LeaveDetailsState(
                leaveDetailsStatus: LeaveDetailsStatus.loading),
            const LeaveDetailsState(
                leaveDetailsStatus: LeaveDetailsStatus.success),
          ]));
    });
  });
}
