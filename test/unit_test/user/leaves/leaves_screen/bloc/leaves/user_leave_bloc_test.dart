  ...
  group('User Leaves Test', () {
    group('User Leave initial fetch data test', () {
      ...
      test('Load initial leave success test', () {
        ...
        when(
          leaveRepo.leaves(uid: employeeId, leaveType: LeaveType.casualLeave),
        ).thenAnswer(
          (_) async => PaginatedLeaves(leaves: [casualLeave], lastDoc: lastDoc),
        );
        when(
          leaveRepo.leaves(uid: employeeId, leaveType: LeaveType.urgentLeave),
        ).thenAnswer(
          (_) async => PaginatedLeaves(leaves: [urgentLeave], lastDoc: lastDoc),
        );
      ...
      test('Add applied leaves on leave list test', () {
        when(
          leaveRepo.fetchLeave(leaveId: casualLeave.leaveId),
    ...
    group('User Leave fetch more data success test', () {
      ...
      test('fetch more data leave success test for Casual leave', () {
        when(
          ...
          (_) async =>
              PaginatedLeaves(leaves: [casualLeave], lastDoc: moreDataLastDoc),
        );
        when(
          leaveRepo.leaves(uid: employeeId, leaveType: LeaveType.urgentLeave),
        ).thenAnswer(
      ...
      test('Update leaves on list test', () {
        when(
          leaveRepo.fetchLeave(leaveId: initialLeaveWithChange.leaveId),
